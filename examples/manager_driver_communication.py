...
if __name__ == "__main__":
    ...
    manager._add_inform_driver_path_change(
        jid=str(driver_1.jid), data=PathChangeBody(path=[1, 4, 10])
    )
    manager._add_inform_driver_path_change(
        jid=str(driver_2.jid), data=PathChangeBody(path=[2, 6, 7])
    )
    manager._add_inform_driver_path_change(
        jid=str(driver_3.jid), data=PathChangeBody(path=[3, 1, 12])
    )
    sleep(10)
    ...
    manager._add_inform_driver_path_change(
        jid=str(driver_3.jid), data=PathChangeBody(path=[13, 5, 3])
    )
    sleep(5)
    logger.info("Request all drivers data")
    manager._add_request_all_drivers_data()
    ...
    while (
        manager.is_alive()
        or driver_1.is_alive()
        or driver_2.is_alive()
        or driver_3.is_alive()
    ):
        try:
            sleep(1)
