...
class DriverAgent(agent.Agent):
    ...
    async def setup(self):
        ...
        self.add_behaviour(
            self.receive_request_driver_data,
            self.receive_request_driver_data_template,
        )
        self.add_behaviour(
            self.receive_inform_path_change,
    ...
    async def _setup_subscribe_to_manager(self):
        self.subscribe_to_manager = self.SubscribeToManager(
            period=self._config.DRIVER_SUBSCRITPION_CHECK_PERIOD
        )
    ...
    class InformDriverData(BaseOneShotBehaviour):
        ...
        async def run(self):
            ...
            msg.body = json.dumps(
                ReceiveDriverDataBody(**self.agent._get_state()),
    ...
    class SubscribeToManager(BasePeriodicBehaviour):
        ...
        async def run(self):
            ...
            if (
                manager_jid not in self.agent.presence.get_contacts()
                or self.agent.presence.get_contact(manager_jid).get(
                    "subscription", None
                )
                == "none"
            ):
                self.agent.presence.unsubscribe(self.agent._config.MANAGER_JID)
                self.agent._logger.info(
                    f"{self.agent._config.MANAGER_JID}"
                    f"have not accepted subscription."
                )
                self.agent._logger.info(
                    f"Subscribing to {self._config.MANAGER_JID}."
                )
        ...
        def on_unsubscribe(self, jid: str):
            self.agent._logger.info(
                f"Agent {jid} asked for removeing subscription."
            )
            self.presence.approve(jid)
