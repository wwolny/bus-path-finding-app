...
from do_celu.behaviours import BaseCyclicBehaviour, BaseOneShotBehaviour
...
class MathematicianAgent(agent.Agent):
    ...
    class ReceiveRequestBestPaths(BaseCyclicBehaviour):
        ...
        async def run(self):
            ...
            if msg:
                self._logger.debug(
                    f"ReceiveRequestBestPath msg received with body: "
                    f"{msg.body}"
                )
                ...
                self._logger.debug(
                    f"Sender: {sender} - Mathematician data: {vars(data)}"
                )
        ...
        async def on_end(self):
            self._logger.debug(
                f"ReceiveRequestBestPath ended with status: "
    ...
    class InformBestPaths(BaseOneShotBehaviour):
        ...
        def tsp(
            self, needed: List[Tuple[int, int]], busroutes: List[List[int]]
        ):
        ...
        async def run(self):
            ...
            busroutes = self.tsp(
                needed=self._new_rides, busroutes=self._bus_routes
            )
            msg = ReceiveBestRoutesMessage(to=self._receiver_jid)
            msg.body = json.dumps(
                ReceiveBestPathsBody(best_paths=busroutes),
                cls=DataclassJSONEncoder,
            )
            try:
                await self.send(msg)
                self._logger.debug(
                    "Sending the best routes! - Best Routes: {0}".format(
                        busroutes
                    )
                )
    ...
    def _add_inform_best_paths(
        self, sender: str, request: RequestBestPathsBody
    ):
        self.inform_best_paths = self.InformBestPaths(
            receiver_jid=sender,
